...
void Testbench::do_median_mean_filter() {
    ...
    for (y = 0; y != height; ++y) {
        ...
        if (y == 0) {
            ...
            for (v = -yBound; v != yBound + adjustY; ++v) {   //-1, 0, 1
                for (u = -xBound; u != xBound + adjustX; ++u) { //-1, 0, 1
                    ...
                    initiator.write_to_socket(MEDIAN_FILTER_R_ADDR, mask, data.uc, 4, delay);
                    m_qk.inc(delay);
        ...
        while(!done){
            delay = m_qk.get_local_time();
            initiator.read_from_socket(MEDIAN_FILTER_CHECK_ADDR, mask, data.uc, 4, delay);
            m_qk.inc(delay);
            if (m_qk.need_sync()) m_qk.sync();
        ...
        delay = m_qk.get_local_time();
        initiator.read_from_socket(MEDIAN_FILTER_RESULT_ADDR, mask, data.uc, 4, delay);
        m_qk.inc(delay);
        if (m_qk.need_sync()) m_qk.sync();
        total = data.sint;
        //debug
    ...
    for (y = 0; y != height; ++y) {
        ...
        if (y == 0) {
            ...
            for (v = -yBound; v != yBound + adjustY; ++v) {   //-1, 0, 1
                for (u = -xBound; u != xBound + adjustX; ++u) { //-1, 0, 1
                    ...
                    mask[3] = 0;
                    delay = m_qk.get_local_time();
                    initiator.write_to_socket(MEAN_FILTER_R_ADDR, mask, data.uc, 4, delay);
                    m_qk.inc(delay);
                    if (m_qk.need_sync()) m_qk.sync();
                }
            }
        ...
        while(!done){
            delay = m_qk.get_local_time();
            initiator.read_from_socket(MEAN_FILTER_CHECK_ADDR, mask, data.uc, 4, delay);
            m_qk.inc(delay);
            if (m_qk.need_sync()) m_qk.sync();
            output_num = data.sint;
            if(output_num>0) done=true;
        ...
        delay = m_qk.get_local_time();
        initiator.read_from_socket(MEAN_FILTER_RESULT_ADDR, mask, data.uc, 4, delay);
        m_qk.inc(delay);
        if (m_qk.need_sync()) m_qk.sync();
        total = data.sint;
        //debug
